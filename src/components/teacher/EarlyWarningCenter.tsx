...
const PRIORITY_STYLES: Record<MentorAttentionPriority, { dot: string; badge: string; border: string }> = {
  ...
  high: { dot: 'bg-amber-600', badge: 'bg-amber-100 text-amber-800', border: 'border-amber-200' },
...
export default function EarlyWarningCenter({ students, loading, error, onSelectStudent, onRetry }: Props) {
  const attentionStudents = students
    .filter(student => student.alerts.priority !== 'low' && student.alerts.alerts.length > 0)
    .sort((a, b) => (
  ...
  return (
    <Card className="overflow-hidden border-indigo-100 !p-0">
      <div className="flex flex-col gap-2 border-b border-indigo-100 bg-slate-50 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 className="font-bold text-gray-800">Bugün Dikkat Gerektirenler</h3>
          <p className="mt-0.5 text-xs text-gray-500">Gözlenen uyarılara göre takip öncelikleri.</p>
        </div>
      ...
      {loading ? (
        <div role="status" className="flex items-center justify-center gap-3 p-5 text-sm text-gray-500">
          <Spinner /> Öncelikler hazırlanıyor...
        </div>
      ) : error ? (
        <div className="p-6 text-center">
          <p role="alert" className="font-medium text-red-700">Erken uyarılar hazırlanamadı.</p>
        ...
        <div className="px-5 py-3">
          <p className="text-sm font-semibold text-gray-700">Şu anda dikkat gerektiren bir uyarı yok.</p>
          <p className="mt-1 text-xs text-gray-500">Yeni veriler geldikçe öncelikler otomatik güncellenir.</p>
        </div>
      ) : (
                ...
                <button type="button" onClick={() => onSelectStudent(student.id)} className="mt-3 block w-full text-left">
                  <ul className="space-y-1.5 text-xs leading-5 text-gray-600">
                    ...
                    {student.alerts.alerts.length > visibleAlerts.length && (
                      <li className="font-medium text-gray-500">+{student.alerts.alerts.length - visibleAlerts.length} uyarı daha</li>
                    )}
                  </ul>
                </button>
